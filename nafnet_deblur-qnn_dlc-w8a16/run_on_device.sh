...
usage() {
  cat <<'EOF'
Usage:
  ./run_on_device.sh INPUT_IMAGE [OUTPUT_IMAGE]
...
Environment variables:
  ...
  REBUILD_CONTEXT=1  regenerate the optimized HTP context
EOF
}
...
OUTPUT_IMAGE=${2:-"$SCRIPT_DIR/output_qnn_w8a16.png"}
MODEL_PATH="$SCRIPT_DIR/nafnet_deblur.dlc"
REMOTE_DIR=${REMOTE_DIR:-/data/local/tmp/nafnet_deblur_qnn_w8a16}
...
if [[ "$SOC_MODEL" != SM8850 && "$SOC_MODEL" != SM8850L ]]; then
  echo "This optimized config targets SM8850/v81; detected $SOC_MODEL." >&2
  exit 1
fi
...
python3 "$SCRIPT_DIR/prepare_input.py" "$INPUT_IMAGE" \
  --raw "$WORK_DIR/input_uint16.raw" \
  --preview "$WORK_DIR/input_640x360.png" \
  --input-list "$WORK_DIR/input_list_native.txt"
...
push_if_needed "$QNN_SDK_ROOT/lib/aarch64-android/libQnnHtpV81Stub.so"
...
push_if_needed "$QNN_SDK_ROOT/lib/hexagon-v81/unsigned/libQnnHtpV81Skel.so"
...
push_if_needed "$WORK_DIR/input_uint16.raw"
push_if_needed "$WORK_DIR/input_list_native.txt"
"${ADB[@]}" shell "chmod 755 '$REMOTE_DIR/qnn-net-run' '$REMOTE_DIR/qnn-context-binary-generator' '$REMOTE_DIR/qnn-profile-viewer'"
...
CONTEXT_STEM="nafnet_w8a16_htp_v81_maxvtcm_o3_${MODEL_HASH}"
CONTEXT_NAME="$CONTEXT_STEM.bin"
...
if [[ "$CONTEXT_EXISTS" != yes || "${REBUILD_CONTEXT:-0}" == 1 ]]; then
  echo "Generating O3/max-VTCM HTP context; this takes about one minute..."
...
REMOTE_OUTPUT="output_htp_w8a16_$RUN_ID"
echo "Running $NUM_INFERENCES inference(s) on $SERIAL..."
"${ADB[@]}" shell "cd '$REMOTE_DIR' && export LD_LIBRARY_PATH='$REMOTE_LD' && export ADSP_LIBRARY_PATH='$REMOTE_ADSP' && ./qnn-net-run --backend libQnnHtp.so --retrieve_context 'context_htp/$CONTEXT_NAME' --config_file htp_netrun_o3_config.json --input_list input_list_native.txt --output_dir '$REMOTE_OUTPUT' --profiling_level basic --perf_profile burst --num_inferences '$NUM_INFERENCES' --keep_num_outputs 1 --use_native_input_files --use_native_output_files --shared_buffer --log_level warn"

LOCAL_RESULT="$SCRIPT_DIR/results/${RUN_ID}_htp_w8a16"
...
RAW_OUTPUT="$LOCAL_RESULT/Result_0/deblurred_image_native.raw"
python3 "$SCRIPT_DIR/decode_output.py" "$RAW_OUTPUT" "$OUTPUT_IMAGE"

echo "Device: $SERIAL ($SOC_MODEL)"
